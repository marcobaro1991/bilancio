defmodule Bilancio.Redis.Conn do
  @moduledoc nil

  @url_redis Application.compile_env!(:bilancio, :redis)[:connection_url]

  use GenServer

  def start_link([]) do
    GenServer.start_link(__MODULE__, @url_redis, name: :bilancio_redis)
  end

  def start_link(opts) do
    GenServer.start_link(__MODULE__, opts, name: :bilancio_redis)
  end
  ...
  def execute_command(cmd, _url) do
    GenServer.call(:bilancio_redis, {:command, cmd})
  end
  ...
  def execute_commands_in_transaction(commands) do
    GenServer.call(:bilancio_redis, {:transaction, commands})
  end
